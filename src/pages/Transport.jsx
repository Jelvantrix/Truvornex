...
import { Car, Package, Truck, Key, Plus, MapPin, Calendar, Users, ArrowRight, Search, X } from 'lucide-react';
...
const INPUT_CLS = 'input-lightning w-full px-4 py-3.5 rounded-xl outline-none text-sm transition-all';
const INPUT_STYLE = { backgroundColor: 'var(--color-surface-high)', border: '1px solid var(--color-border)', color: 'var(--color-text)' };
    ...
    return (
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
                ...
                {SEGMENTS.map(s => {
                    ...
                    return (
                        <button key={s.key} onClick={() => setTab(s.key)}
                            className="btn-lightning flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all"
                            style={{
            ...
            <form onSubmit={handleSearch} className="flex gap-2 mb-5">
                ...
                <button type="submit" className="btn-lightning px-3 py-2 rounded-xl"
                    style={{ backgroundColor: 'var(--color-primary)', color: 'var(--color-on-primary)' }}>
            ...
            {loading ? (
                ...
                <div className="space-y-3">
                    {rides.map(ride => {
                        ...
                        return (
                            <div key={ride.id} className="card-premium rounded-2xl p-4 shimmer hover-lift"
                                ...
                                <div className="flex items-start justify-between gap-3">
                                    ...
                                    {!isDriver && (
                                        <button
                                            ...
                                            className="btn-lightning px-3 py-1.5 rounded-xl text-xs font-semibold shrink-0 transition-opacity"
                                            style={{
            ...
            {createOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}
                    ...
                    <div className="card-premium rounded-2xl p-5 w-full max-w-md shadow-xl overflow-y-auto max-h-[90vh]"
                        ...
                        <div className="flex gap-1.5 mb-4 flex-wrap">
                            {SEGMENTS.map(s => (
                                <button key={s.key} onClick={() => setForm(f => ({ ...f, type: s.key }))}
                                    className="btn-lightning flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold"
                        ...
                        <div className="flex gap-2 mt-4">
                            <button onClick={() => setCreateOpen(false)} className="btn-lightning flex-1 py-2 rounded-xl text-sm font-semibold"
                                style={{ backgroundColor: 'var(--color-surface-high)', color: 'var(--color-text-muted)', border: '1px solid var(--color-border)' }}>
                                Cancel
                            </button>
                            <button onClick={create} disabled={saving}
                                className="btn-lightning flex-1 py-2 rounded-xl text-sm font-semibold transition-opacity"
