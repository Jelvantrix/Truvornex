...
const SimonInsightsWidget = memo(function SimonInsightsWidget() {
    ...
    return (
        <section className="card-lightning-ai" style={{
...
const LiveNeighborhoodStats = memo(function LiveNeighborhoodStats({ location }) {
    ...
    useEffect(() => {
        ...
        function connect() {
            ...
            const es = new EventSource(`/api/realtime/neighborhood-stats?lat=${lat}&lng=${lng}`);
            eventSourceRef.current = es;

            es.addEventListener('neighborhood_stats', (e) => {
                try {
...
export default function Home() {
    ...
    // Single geolocation instance shared across Home and all child components
    ...
    // Reverse geocode using already-resolved location — no duplicate GPS request
    useEffect(() => {
        if (!location || location.length !== 2) return;
        const [lat, lng] = location;
    ...
    return (
        <div className="w-full flex flex-col gap-4 md:gap-7">
            ...
            {/* ── Hero ──────────────────────────────────────────────────── */}
            <section className="section-lightning-ai pt-1 relative flex flex-col items-center text-center">
                {/* ambient */}
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 mb-4 w-full max-w-xs sm:max-w-none sm:w-auto" style={anim(0.1)}>
                    <button
                        ...
                        className="btn-lightning flex items-center justify-center gap-1.5 px-5 rounded-xl font-semibold transition-all"
                    ...
                    <button
                        ...
                        className="btn-lightning-ai flex items-center justify-center gap-1.5 px-5 rounded-xl font-semibold transition-all"
            ...
            {/* ── Categories ────────────────────────────────────────────── */}
            <section className="section-lightning">
                <SectionHeader title="Browse by Category" href="/services" />
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-1.5">
                    {CATEGORIES.map((cat, i) => (
                        <Link key={cat.slug} to={`/category/${cat.slug}`}
                            className="card-lightning-subtle flex flex-col items-center gap-1.5 rounded-xl text-center"
            ...
            <section className="section-lightning">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    <div className="col-span-full text-center py-8 text-xs" style={{ color: 'var(--color-text-subtle)' }}>
                        Loading events... (Dynamic data from API)
                    </div>
                </div>
            ...
            {/* ── Service Bundles ───────────────────────────────────────── */}
            <section className="section-lightning" style={anim(0.1)}>
                <SectionHeader title="Service Bundles" href="/bundles" label="All bundles" />
                <div className="card-lightning-subtle rounded-xl overflow-hidden" style={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)' }}>
                    ...
                    <div className="px-3.5 py-4 text-center text-xs" style={{ color: 'var(--color-text-subtle)' }}>
                        Loading bundles... (Dynamic data from API)
                    </div>
            ...
            {/* ── Marketplace ───────────────────────────────────────────── */}
            <section className="section-lightning" style={anim(0.14)}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    {[
                        ...
                        <div key={i} onClick={() => navigate(item.href)}
                            className="card-lightning-subtle rounded-xl cursor-pointer relative overflow-hidden"
            ...
            {/* ── Community ─────────────────────────────────────────────── */}
            <section className="section-lightning" style={anim(0.18)}>
                <div className="flex items-center justify-between mb-2.5">
            ...
            {/* ── Financial Overview ────────────────────────────────────── */}
            <section className="section-lightning" style={anim(0.22)}>
            ...
            {/* ── How it Works ──────────────────────────────────────────── */}
            <section className="section-lightning rounded-xl relative overflow-hidden"
                    ...
                    {(liveProviders.length > 0 ? liveProviders : FALLBACK_PROVIDERS).map((p, i) => {
                        ...
                        return (
                            <Link key={p.id || i} to={href}
                                className="card-lightning rounded-xl overflow-hidden block group"
            ...
            {/* ── Quick Links ───────────────────────────────────────────── */}
            <section className="section-lightning grid grid-cols-1 sm:grid-cols-2 gap-2" style={anim(0.32)}>
                <Link to="/nearby"
                    className="card-lightning-subtle flex items-center justify-between rounded-xl transition-all duration-200 group"
                ...
                <Link to="/ai"
                    className="card-lightning-ai flex items-center justify-between rounded-xl transition-all duration-200 group relative overflow-hidden"
            ...
            {/* ── Provider CTA ──────────────────────────────────────────── */}
            <section className="section-lightning-ai rounded-xl relative overflow-hidden"
                ...
                <div className="relative flex flex-col sm:flex-row items-start gap-4">
                    <div className="flex-1">
                        ...
                        <button onClick={() => navigate('/provider')}
                            className="btn-lightning-ai inline-flex items-center gap-1.5 rounded-lg text-[12px] font-bold transition-all"
