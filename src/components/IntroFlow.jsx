...
function generateParticles(W, H) {
    // Position at right-center: 75% horizontal, 50% vertical
    const isCompact = W < 600;
    // Scale factor - make logo larger and crisper
    const scale = Math.min(W * (isCompact ? 0.52 : 0.44), H * (isCompact ? 0.52 : 0.44)) / 32;
    const ox = W * 0.75;   // right-center horizontally (75%)
    const oy = H * 0.50;   // center vertically (50%)
    const STEP     = 0.25;       // tighter grid spacing for HD crispness
    const HALF_T   = 1.2;        // slightly thinner bands for sharper edges
    ...
    return pts.map(p => ({
        tx:          p.tx,
        ty:          p.ty,
        x:           p.tx + (Math.random() - 0.5) * W * 0.4,
        y:           p.ty + (Math.random() - 0.5) * H * 0.4,
        vx:          0,
        vy:          0,
        size:        1.0 + Math.random() * 1.2,  // smaller particles = crisper look
...
function LogoParticles() {
    ...
    useEffect(() => {
        ...
        const resize = () => {
            ...
            // Use higher DPR for crisp HD rendering, capped at 2 for performance
        ...
        const spawnRipple = (x, y) => {
            const maxR = Math.min(state.viewport.width, state.viewport.height) * 0.45;
            state.ripples.push({ x, y, r: 0, maxR, str: 8 });
        ...
        const initRipple = setTimeout(() => {
            const cx = state.viewport.width * 0.75;
            const cy = state.viewport.height * 0.50;
            state.ripples.push({ x: cx, y: cy, r: 0, maxR: Math.min(state.viewport.width, state.viewport.height) * 0.50, str: 12 });
        }, 900);
        const autoRippleId = setInterval(() => {
            const cx = state.viewport.width * 0.75;
            const cy = state.viewport.height * 0.50;
            state.ripples.push({ x: cx, y: cy, r: 0, maxR: Math.min(state.viewport.width, state.viewport.height) * 0.48, str: 9 });
            if (state.ripples.length > 4) state.ripples.shift();
        }, 4500);

        // Physics constants - tuned for crisper look
        const STIFFNESS     = 0.042;  // slightly faster return for snappier feel
        const DAMPING       = 0.78;   // slightly less damping for crisper motion
        const REPEL_RADIUS  = 100;    // slightly smaller radius
        const REPEL_STRENGTH = 5.0;   // slightly less strength
        ...
        const animate = (now) => {
            ...
            for (const p of state.particles) {
                ...
                const pulse = Math.sin(now * 0.00075 + p.phase) * 0.1;
                const alpha = Math.max(0.08, Math.min(0.72, p.baseOpacity + pulse));
