...
export default function ZoneHeatmap() {
    ...
    useEffect(() => {
        ...
        function startPolling() {
            ...
            fetchFallback();
            pollRef.current = setInterval(fetchFallback, 30_000);
        }
        ...
        function connect() {
            ...
            try {
                ...
                es.onerror = () => {
                    setConnected(false);
                    es.close();
                    startPolling(); // SSE down — fall back to polling
                    reconnectTimer = setTimeout(connect, 30_000); // retry SSE every 30s
                };
            } catch (_) {
                setConnected(false);
                startPolling();
        ...
        // If SSE hasn't delivered data within 5s, start polling immediately
        ...
        <section style={{ borderRadius: 14, overflow: 'hidden', border: '1px solid var(--color-border)', backgroundColor: 'var(--color-surface)', boxShadow: 'var(--shadow-sm)' }}>
            ...
            <div style={{ padding: 20, textAlign: 'center' }}>
                <p style={{ fontSize: 11, color: 'var(--color-text-subtle)' }}>
                    {connected ? 'No zones available' : 'Connecting to live data...'}
                </p>
            </div>
        </section>
    );
