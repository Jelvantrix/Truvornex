...
| `OPENROUTER_API_KEY` | `sk-or-v1-...` | Optional: Get from [openrouter.ai](https://openrouter.ai) — enables Simon AI |
| `SMTP_HOST` | `smtp.gmail.com` | Optional: Or your SMTP provider |
| `SMTP_PORT` | `587` | Optional: |
| `SMTP_USER` | `your@email.com` | Optional: |
| `SMTP_PASS` | (app password) | Optional: Use Gmail App Password, not your main password |
| `SMTP_FROM` | `noreply@truvornex.com` | Optional: |
| `CLERK_SECRET_KEY` | `sk_live_...` | Optional: Get from Clerk Dashboard |
| `CLERK_PUBLISHABLE_KEY` | `pk_live_...` | Optional: Get from Clerk Dashboard |
...
**Important Notes:**
- `OPENROUTER_API_KEY` is optional — Simon AI will run in degraded mode without it
- SMTP variables are optional — email features will be disabled without them
- Clerk keys are optional — app will use custom auth without them
- Only `DATABASE_URL`, `SESSION_SECRET`, `TRUST_PASSPORT_SECRET`, `SIMON_SYSTEM_TOKEN`, and `MFA_ENCRYPTION_KEY` are required
...
**IMPORTANT:** Your app uses PostGIS for spatial zone detection and location-based features. This MUST be enabled on Railway Postgres:
...
3. Verify installation:
   ...
   ```
4. Railway Postgres has PostGIS pre-installed — you just need to enable it.

**Note:** If location features aren't working, PostGIS extension is likely not enabled. The application will fail gracefully but neighborhood stats and provider proximity features will not function.
