...
async function initDb() {
    try {
        ...
        // Make password_hash nullable for existing tables (PostgreSQL doesn't support ALTER COLUMN ... DROP NOT NULL in all versions)
        try {
            await pool.query(`
                ALTER TABLE users ALTER COLUMN password_hash DROP NOT NULL
            `).catch(() => {
                // If this fails, table might already be nullable, continue
            });
        } catch (e) {
            // Ignore constraint modification errors
        }
        ...
        // In development, allow server to start even if DB init fails
        if (!isProd) {
            console.warn('Continuing without full database initialization (development mode)');
        } else {
            throw err; // In production, fail hard if DB init fails
        }
    }
}
    ...
    try {
        ...
        if (err.code === '23502') {
            return res.status(400).json({ 
                error: 'Missing required field. Please provide all required information.',
                code: 'MISSING_FIELD'
            });
        }
        ...
        if (err.code === '22001') {
            return res.status(400).json({ 
                error: 'Input too long. Please shorten your input.',
                code: 'INPUT_TOO_LONG'
            });
        }
...
app.get('/api/realtime/neighborhood-stats',      handleNeighborhoodStatsStream); // public — no auth
...
if (isProd) {
    ...
    initDb().then(async () => {
        ...
        const server = app.listen(PORT, '0.0.0.0', () => {
            console.log(`✓ Truvornex production server running on port ${PORT}`);
            console.log(`✓ Environment: ${process.env.NODE_ENV}`);
            console.log(`✓ Database connected`);
            console.log(`✓ PostGIS enabled for location features`);
        });
