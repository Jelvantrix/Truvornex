...
// ── SSE: Neighborhood stats stream (public — no auth needed) ─────────────────
export async function handleNeighborhoodStatsStream(req, res) {
    ...
    const sendStats = async () => {
        ...
        try {
            const [onlineRes, acceptingRes, zoneRes, categoriesRes] = await Promise.all([
                pool.query(
                    `SELECT COUNT(*)::int AS count FROM provider_presence pp
                     JOIN providers p ON p.user_id = pp.provider_id
                     WHERE pp.is_online = true AND pp.last_heartbeat > $1
                       AND ST_DWithin(p.geolocation, ST_SetSRID(ST_MakePoint($2,$3),4326)::geography, 25000)`,
                    [eightMinutesAgo, lng, lat]
                ),
                pool.query(
                    `SELECT COUNT(*)::int AS count FROM provider_presence pp
                     JOIN providers p ON p.user_id = pp.provider_id
                     WHERE pp.is_online = true AND pp.is_accepting_jobs = true AND pp.last_heartbeat > $1
                       AND ST_DWithin(p.geolocation, ST_SetSRID(ST_MakePoint($2,$3),4326)::geography, 25000)`,
                    [eightMinutesAgo, lng, lat]
                ),
                pool.query(
                    `SELECT demand_index FROM neighborhood_zones
                     WHERE ST_DWithin(geolocation, ST_SetSRID(ST_MakePoint($1,$2),4326)::geography, radius_meters)
                     LIMIT 1`,
                    [lng, lat]
                ),
                pool.query(
                    `SELECT p.category_slug, COUNT(*)::int AS count FROM provider_presence pp
                     JOIN providers p ON p.user_id = pp.provider_id
                     WHERE pp.is_online = true AND pp.last_heartbeat > $1
                       AND ST_DWithin(p.geolocation, ST_SetSRID(ST_MakePoint($2,$3),4326)::geography, 25000)
                     GROUP BY p.category_slug ORDER BY count DESC LIMIT 3`,
                    [eightMinutesAgo, lng, lat]
                ),
            ]);

            const demandIndex = zoneRes.rows.length > 0 ? parseFloat(zoneRes.rows[0].demand_index) : 50;
            sseWrite(res, 'neighborhood_stats', {
                providers_online_now: onlineRes.rows[0].count,
                accepting_jobs_now: acceptingRes.rows[0].count,
                demand_index: demandIndex,
                surge_active: demandIndex > 70,
                top_available_categories: categoriesRes.rows.map(r => r.category_slug),
                timestamp: new Date().toISOString(),
            });
        } catch (err) {
            console.error('[Realtime] neighborhood stats error:', err.message);
        }
    };
