...
router.post('/update', async (req, res) => {
    ...
    try {
        ...
        // Return success even if DB fails - don't break the frontend
        res.json({
            zone: null,
            providers_online_nearby: 0,
            top_categories: [],
            location: { lat, lng },
            db_error: true
        });
...
router.post('/provider/heartbeat', async (req, res) => {
    ...
    try {
        ...
        // Detect current zone — fully parameterized
        ...
        // Upsert into provider_presence — fully parameterized
        await pool.query(
            `INSERT INTO provider_presence (provider_id, lat, lng, geolocation, is_online, is_accepting_jobs, last_heartbeat, current_zone_id, active_booking_id, device_info)
             VALUES ($1, $2, $3, ST_SetSRID(ST_MakePoint($3, $2), 4326)::geography, true, $4, NOW(), $5, $6, $7)
        ...
        // Count active bookings nearby — fully parameterized
...
// GET /api/location/realtime/neighborhood - SSE endpoint for live neighborhood stats (public — no auth needed)
router.get('/realtime/neighborhood', async (req, res) => {
    ...
    const sendStats = async () => {
        // Compute fresh threshold on every call so it doesn't drift
        const eightMinutesAgo = new Date(Date.now() - 8 * 60 * 1000);
        try {
            // Get providers online now
            const { rows: onlineProviders } = await pool.query(
